...
class CreateRidgeLinearRegressionWithKFold:

    def __init__(self, dataSet):
        self.dataSet = dataSet

    def preProcessTrainingSet(self, degree, inputTrainingSet, printParams=False):
        """
    ...
    def prepareModel(self, inputTrainingSet, outputTrainingSet, printModel=False):
        ...
        :return:
        """
        ridge = self.__getModel(printModel)
        ridge.fit(inputTrainingSet, outputTrainingSet)
        return ridge
    ...
    def __preparingForEvaluation__(self, outputTestingSet, outputTrainingSet, trainPredictedSet, testPredictedSet,
                                   model,
                                   isFirstTime):
        """
    ...
    def process(self, numberOfFolds):
        ...
        for trainIndex, testIndex in kfold.split(self.dataSet):
            ...
            if (isFirstTime):
                self.__preparingForEvaluation__(outputTestingSet, outputTrainingSet, trainPredictedSet,
                                                testPredictedSet,
                                                model,
                                                isFirstTime)
                isFirstTime = False
        ...
        LinearRegressionUtility.createValidationCurvAsParam(Ridge(normalize=True), "alpha",
